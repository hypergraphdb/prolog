...
public class JavaLibrary extends Library {
	...
	public Term parse_term_1(Term string)
	{
		try
		{
			return getEngine().toTerm(((Struct)string).getName());
		}
		catch (Exception t)
		{
	...
	protected Struct generateFreshId() {
		return new Struct(getDynamicObjectIdPrefix() + id++);        
	}
	
	public final String getDynamicObjectIdPrefix()
	{
		return "$obj_";
	}
